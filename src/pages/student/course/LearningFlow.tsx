...
export function LearningFlow({ active, hasActivity, hasAssessment }: { active: FlowStep; hasActivity: boolean; hasAssessment: boolean }) {
  ...
  const steps = [
    { id: "learn" as const, label: "Learn it", help: "Start here", icon: BookOpen },
    ...(hasActivity ? [{ id: "do" as const, label: "Do it", help: "After learning · either order", icon: ListChecks }] : []),
    ...(hasAssessment ? [{ id: "assess" as const, label: "Assess it", help: "After learning · either order", icon: ClipboardCheck }] : []),
  ];
      ...
      {steps.map((step, index) => {
        ...
        return (
          <div key={step.id} className="contents">
            {index === 1 && <ChevronRight size={15} className="mx-0.5 shrink-0 text-ink-300 sm:mx-2" aria-hidden="true" />}
            {index > 1 && <span className="mx-1 text-xs text-ink-400" aria-hidden="true">/</span>}
            <Link to={destination?.available ? destination.href : location.pathname} onClick={e=>{if(!destination?.available)e.preventDefault();}} aria-disabled={!destination?.available} title={destination&&!destination.available?destination.reason:undefined} aria-current={current ? "step" : undefined} className={`flex min-w-0 flex-1 items-center gap-2 rounded-lg px-2.5 py-2 transition-colors focus-visible:ring-2 focus-visible:ring-brand-500 ${current ? "bg-brand-700 text-white shadow-sm" : complete ? "bg-success-50 text-success-800" : "text-ink-600"}`}>
              ...
              <span className="min-w-0"><span className="whitespace-nowrap text-xs font-semibold">{step.label}</span><span className={`hidden text-[10px] sm:block ${current ? "text-brand-100" : "text-ink-500"}`}>{step.help}</span></span>
            </Link>
          </div>
        );
      })}
